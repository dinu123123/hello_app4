                 ...
                 @localEvent = Event.find_by_sql(['SELECT * FROM events where events."DRIVER_id" = ? and  events."client_id" = ?
                   ...
                   #fill in an array with payment dates
                           ...
                           @localEvent.each_with_index do |event,i|
                             if(event.expected_date and event.expected_date.strftime("%U").to_i >= @period_start and event.expected_date.strftime("%U").to_i <= @period_end and @arrayWeeklyTruckExpense[event.DATE.strftime("%U").to_i+1][j+1])
