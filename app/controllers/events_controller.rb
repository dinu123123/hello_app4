                             ...
                             Driver.all.each_with_index do |driver,t|
                                                     ...
                                                     #check if all invoiced_trips_for_dispatcher and driver have the same truck_id  
                                                      ...
                                                      if  (@invoiced_trips_for_dispatcher != nil and 
                                                          ...
                                                          fuel_expenses.trsdate ASC, fuel_expenses.trstime ASC", "Diesel","diesel","DIESEL", 
                                                          @invoiced_trips_for_dispatcher[0].truck_id, @date_from1-30, 
                                                          @date_to1+1])

                                                          @size_base = FuelExpense.find_by_sql(["SELECT * FROM fuel_expenses where  
                                                          (fuel_expenses.product = ? or fuel_expenses.product = ? or fuel_expenses.product = ?) and 
                                                          fuel_expenses.truck_id = ? AND
                                                          fuel_expenses.trsdate > ? AND fuel_expenses.trsdate < ? ORDER BY 
                                                          fuel_expenses.trsdate ASC, fuel_expenses.trstime ASC","Diesel","diesel","DIESEL", 
                                                          @invoiced_trips_for_dispatcher[0].truck_id, @date_from1, 
                                                          @date_to1+1]).size
...
@arrayWeeklyTruckExpense[0][8] = "CashFlow".to_s
 ...
 expected_paid_invoices = @totalInvoicedTrips
                 ...
                 paid_invoices = Invoice.find_by_sql(['SELECT SUM("total_amount") AS sum1 FROM invoices where  invoices."paid" = ? AND
                  ...
                  (invoices."collection_date" IS NULL AND invoices."ddate" > ? AND invoices."ddate" <  ? ))', true, @date_from1-1, @date_to1+1, @date_from1-1, @date_to1+1])[0].sum1
